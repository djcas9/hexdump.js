  ...
  function Hexdump(data, options) {
    ...
    switch (self.options.base) {
      ...
      case 16:
        ...
        self.baseConvert = function (characters) {
          for (var i = 0; i < characters.length; i++) {
            return self.addPadding(
              characters[i].charCodeAt(0).toString(16),
              self.padding.hex
            );
          }
        };
      ...
      case 2:
        ...
        self.baseConvert = function (characters) {
          for (var i = 0; i < characters.length; i++) {
            var ddx = characters[i].charCodeAt(0),
              r = "";

            for (var bbx = 0; bbx < 8; bbx++) {
              r = (ddx % 2) + r;
              ddx = Math.floor(ddx / 2);
            }
    ...
    self.nullCount =
      self.options.width - self.data[self.data.length - 1].length;
    ...
    for (var i = 0; i < self.hexdump.length; i++) {
      if (self.options.lineNumber) {
        ...
        if (self.options.html) {
          self.output +=
            '<span id="line-number">' + tempLineNumberStyle + "</span>";
        } else {
  ...
  Hexdump.prototype.process = function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (self.options.html) {
        ...
        if (self.hex) {
          ...
          hexArray.push(
            '<span data-hex-id="' + self.hexCounter + '">' + code + "</span>"
          );
        }
        ...
        stringArray.push(
          '<span data-string-id="' +
            self.hexCounter +
            '">' +
            self.checkForNonPrintable(data[i]) +
            "</span>"
        );
      } else {
        var code = self.baseConvert(data[i]);
    ...
    if (stringArray.length < self.options.width) {
      ...
      for (var i = 0; i < stringAmount; i++) {
        ...
        if (self.options.html) {
          nullString =
            '<span data-string-null="true">' +
            self.options.style.stringNull +
            "</span>";
        } else {
